...
int main() {
	...
	int location_pro = 0;//归属地省份号
	int iCallNum = 100000;        //遍历开始序号
	int iList1 = 100000;  //本节点序列号
	int FileNO = 0;          //文件序中列号
	int file_num = 1;                 //每个省生成文件数量
	int iSend = 10000;          //每个文件结尾记录的条数\每个文件条数
	...
	//单个文件31个省遍历
	int all_visitpro[32] = {22, 23, 24, 25, 27, 28, 29, 471, 351, 551, 431, 791, 871, 891, 898, 931, 951, 971, 991};
	...
	int all_prin[100] = {10, 20, 21, 22, 23, 24, 510, 27, 28, 29, 311, 351, 371, 431, 451, 471, 531, 551, 571, 591, 731,
	...
	int pro_bill_percent[100] = {373, 640, 213, 91, 359, 69, 2004, 414, 840, 221, 304, 78, 348, 88, 70, 80, 163, 418, 279, 190,
	...
	string ip_last_tw_val = "40";
	string chFilenametmp_s = "./~LNIG_%s%03d.%05d." + ip_last_tw_val;
	string chFilename_s = "./LNIG_%s%03d.%05d." + ip_last_tw_val;

	int chose_how_many_visit_pro = 19;

	while (vistpro_id < chose_how_many_visit_pro) {

		visit_pro = all_visitpro[vistpro_id];
		printf("file%d:\n", vistpro_id, visit_pro);
		...
		while (FileNO < file_num)//一个省生成多个文件
		{
			...
			sprintf(chStart, "10^46011%03d^46011000^%05d^%s0000^10\n", visit_pro,
					FileNO,
					chHour);
			std::cout << chFilenametmp << std::endl;
			...
			if (!(fp = fopen(chFilenametmp, "w"))) {
				...
				while (i < 31)   //31个省记录
				{
					location_pro = all_prin[i];

					printf("%d---%d\n", location_pro, visit_pro);
					while (iProCount < pro_bill_percent[j])//每个省记录条数
					{
						memset(chBill, 0, sizeof(chBill));
						sprintf(chBill,
