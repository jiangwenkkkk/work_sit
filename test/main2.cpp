...
int  main()
{
	int iOutPro=0;//�ݷþ�ʡ�ݺ�
	int iInPro=0;//������ʡ�ݺ�
	int iCallNum = 100000;        //������ʼ���
	int iList1 =   100000;  //���ڵ����к�
	int iFileNO=0;          //�ļ������к�
	int iCount=10;                 //ÿ��ʡ�����ļ�����
	int iSend =50000;          //ÿ���ļ���β��¼������\ÿ���ļ�����

	char chTime[16];
	char chHour[16];
	char chBill[1024];   //������¼����
	char chStart[1024];  //�ļ�ͷ
	char chEnd[1024];    //�ļ�β
	char chFilename[64];
	...
	int iList111[100]={10,20,21,22,23,24,25,27,28,29,311,351,371,431,451,471,531,551,571,591,731,771,791,851,871,891,898,931,951,971,991};
	char chIMIS111[80][22]={"460110013831570","460110403884278","460110137120718","460110031485677","460037951848508","460110091515241","460110156971421","460110350853957","460110521746472","460110583474276","460110723772240","460030258165588","460110903996310","460110102047593","460110797761375","460110074726321","460030995412157","460110230369902","460110195005574","460110260847395","460110382610279","460110674593373","460110721024149","460110530533813","460110550248202","460110570509095","460110480900711","460110603299887","460031207119873","460110620052694","460110643913275"};
	int iCountNum[100]={1866,3202,1066,456,1795,346,10020,2070,4201,1106,1521,388,1740,440,351,402,817,2089,1395,951,3708,3788,2295,1069,641,64,309,1209,251,125,319};
	...
	time_t tt = time(NULL);//�컰��ʱ�侫ȷ���֣���ֹ�ظ�
	tm* t= localtime(&tt);
	...
	string ip_last_tw_val = last_ip_two();
	string chFilenametmp_s = "/data1/sr/roam_in/file/~LNIG_%s%03d.%05d."+ip_last_tw_val;
	string chFilename_s = "/data1/sr/roam_in/file/LNIG_%s%03d.%05d." + ip_last_tw_val;
	while(i<4)
	{
		iOutPro = iList[i];
		printf("file%d:\n",i,iOutPro);
		while(iFileNO < iCount)//һ��ʡ���ɶ���ļ�
		{
			iFileNO++;
  	  ...
  	  else
  	  {
				...
				while (i<31)   //31��ʡ��¼
				{
					...
					printf("%d---%d\n",iInPro,iOutPro);
					while(iProCount<iCountNum[j])//ÿ��ʡ��¼����
					{
						memset(chBill,0, sizeof(chBill));						
						sprintf(chBill,"1^18938888888^%s^6201482934%d^255.255.0.1^CTLTE^%s^%d^100000^%s^1^0^%d^%d^A^1000^1000^20180710170110^30^20180710170140^000000000^^460111A00091^%d^192.168.0.0^^%s^\n",
						chIMIS111[i],
						iList1,
						chChargid[0],
						iIMIS[0],
				...
				fputs(chEnd,fp);//д������
				fclose(fp);
				if(!rename(chFilenametmp,chFilename))//ɾ���ɹ�
				{
